...
`temday/full` 会安装已发布的非可变插件，包含格式解析、locale 展示、时区、时长、日历、查询、集合与兼容 API。`badMutable` 会改变不可变方法的行为，仍需显式安装。
...
```html
<!-- unpkg -->
<script src="https://unpkg.com/temday@1.0.0"></script>

<!-- jsDelivr -->
<script src="https://cdn.jsdelivr.net/npm/temday@1.0.0"></script>
```
...
`temday/polyfill` 只会在全局 Temporal 缺失时安装，不会覆盖已有实现。

## 选择入口

只需要默认时区和核心日期能力时，使用 `temday`。需要固定到指定时区的独立工厂时，再额外导入 `temday/context`。
