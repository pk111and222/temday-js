...
`temday-js/full` installs all published non-mutating plugins. It includes custom format parsing, locale display, time-zone APIs, duration, calendar, query, collection, and compatibility helpers. `badMutable` remains explicit because it changes immutable method behavior.
...
After an npm release, unpkg and jsDelivr also resolve the default core UMD through `https://unpkg.com/temday-js@1.0.0` and `https://cdn.jsdelivr.net/npm/temday-js@1.0.0`. Pin an exact version in production.
